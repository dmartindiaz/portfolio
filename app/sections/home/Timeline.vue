...
<template>
  <section id="experience" ref="sectionRef" class="px-4 md:px-8 py-12 md:py-24 bg-white dark:bg-neutral-950 md:h-screen flex flex-col justify-center">
                ...
                <div class="flex items-center gap-3 flex-wrap">
                  <template v-for="(tech, i) in activeItem.mainTechs" :key="tech.name">
                    <div class="flex items-center gap-1.5">
                      ...
                      <span class="text-3xl font-bold text-neutral-900 dark:text-white">{{ tech.name }}</span>
                    </div>
                    <span v-if="i < activeItem.mainTechs.length - 1" class="text-3xl font-bold text-neutral-300 dark:text-white/25">·</span>
                  </template>
                </div>
              </div>

              <UTooltip :delay-duration="100" :arrow="false" :content="{ side: 'top', align: 'center', sideOffset: 4 }" :ui="{ content: 'h-auto ring-0 rounded-xl bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-white/10 p-4 w-72' }">
                ...
                <Marquee :duration="20" pause-on-hover class="[--gap:12px]">
                  <div
                    ...
                    :key="tech.name"
                    class="rounded-xl border border-neutral-200 dark:border-white/10 bg-neutral-100/80 dark:bg-white/5 backdrop-blur-md p-6 flex flex-col gap-3 w-64 shrink-0"
                  >
                    <div class="flex items-center gap-3">
                      <UIcon :name="tech.icon" class="w-7 h-7 shrink-0" />
                      <span class="text-base font-semibold text-neutral-900 dark:text-white">{{ tech.name }}</span>
                    </div>
                    <p class="text-sm text-neutral-500 dark:text-white/50 leading-relaxed">{{ tech.description }}</p>
                  </div>
                </Marquee>
                ...
                <template #content>
                  <div class="flex flex-col gap-2 max-h-52 overflow-y-auto">
                    <p class="text-xs font-semibold tracking-widest uppercase text-neutral-400 dark:text-white/40">{{ $t('timeline.allTechs') }}</p>
                    <div class="flex flex-wrap gap-2">
                      <span
                        v-for="tech in (activeItem?.technologies ?? [])"
                        :key="tech.name"
                        class="flex items-center gap-1.5 px-2.5 py-1 rounded-full border border-neutral-200 dark:border-white/10 bg-neutral-100 dark:bg-white/5 text-xs text-neutral-600 dark:text-white/70"
                      >
            ...
            >
              ...
              <div
                ...
                :class="activeIndex === i
                  ...
                  : 'border-neutral-200 dark:border-white/20 bg-white dark:bg-neutral-950'"
              />

              <!-- Period -->
              <p class="text-sm text-neutral-400 dark:text-white/40 mb-3">
                {{ item.period }}
              </p>

              <!-- Title -->
              <h3
                class="text-xl font-mono font-bold mb-1 text-neutral-900 dark:text-white transition-all duration-500 origin-left inline-block"
              ...
              <!-- Description -->
              <p class="text-sm leading-relaxed text-neutral-500 dark:text-white/70">
                {{ item.description }}
              </p>
              ...
              <div v-if="isMobile && item.technologies?.length" class="flex flex-wrap gap-2 mt-4">
                <span
                  ...
                  :key="tech.name"
                  class="flex items-center gap-1.5 px-2.5 py-1 rounded-full border border-neutral-200 dark:border-white/10 bg-neutral-100 dark:bg-white/5 text-xs text-neutral-600 dark:text-white/60"
                >
                  <UIcon :name="tech.icon" class="w-3.5 h-3.5 shrink-0" />
                  {{ tech.name }}
                </span>
